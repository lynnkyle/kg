...
if not args.no_write:
    os.makedirs(f'absolution/fa/ckpt/{args.model}/{args.data}', exist_ok=True)
    os.makedirs(f'absolution/fa/log/{args.model}/{args.data}', exist_ok=True)
...
file_handler = logging.FileHandler(f'absolution/fa/log/{args.model}/{args.data}/log.log')
...
class MyGo(nn.Module):
    def __init__(self, args, num_ent, num_rel, str_dim,
        ...
        """
        self.ent_emb = nn.Parameter(torch.Tensor(num_ent, str_dim))
        self.rel_emb = nn.Parameter(torch.Tensor(num_rel, str_dim))
    ...
    def init_weights(self):
        ...
        nn.init.normal_(self.pos_head, mean=0.0, std=0.02)
        nn.init.normal_(self.pos_rel, mean=0.0, std=0.02)
        nn.init.normal_(self.pos_tail, mean=0.0, std=0.02)

    def forward(self):
        ent_embs = self.str_drop(self.str_ln(self.ent_emb))
    ...
    if (epoch + 1) % args.valid_epoch == 0:
        ...
        if mrr > best_mrr:
            ...
            torch.save({'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(),
                       ...
                       f'absolution/fa/ckpt/{args.model}/{args.data}/{epoch + 1}.ckpt')
